...
class SystemStatus(BaseModel):
    ...
    model_device: Optional[str] = None       # e.g. "npu", "igpu", "cpu"
    ...
    # Device compatibility (Windows only — Linux/macOS coming soon)
    device_supported: bool = True            # False on unsupported OS or hardware
    processor_name: Optional[str] = None     # Detected CPU or GPU name
```
...
class AgentStepResponse(BaseModel):
    """A single step in the agent's execution history (persisted with the message)."""
    id: int
    type: str              # "thinking" | "tool" | "plan" | "status" | "error"
    label: str             # Human-readable step description
    detail: Optional[str] = None   # Additional detail text
    tool: Optional[str] = None     # Tool function name (for type="tool")
    result: Optional[str] = None   # Tool result summary
    success: Optional[bool] = None
    active: bool = False
    planSteps: Optional[List[str]] = None  # For type="plan"
...
class MessageResponse(BaseModel):
    ...
    agent_steps: Optional[List[AgentStepResponse]] = None  # Tool execution history
...
class DocumentResponse(BaseModel):
    ...
    # "pending" | "indexing" | "complete" | "failed" | "cancelled" | "missing"
  ...
  <Accordion title="GET /api/system/status">
    ...
    **Response:**
    ```json
    {
      ...
      "initialized": true,
      "version": "0.1.0",
      "lemonade_version": "0.18.0",
      "model_size_gb": 17.4,
      "model_device": "npu",
      "model_context_size": 32768,
      "model_labels": ["chat", "code"],
    ...
    `device_supported` is `false` when the OS is not Windows (Linux/macOS support coming soon) or no supported hardware was detected (no AMD Ryzen AI Max and no AMD Radeon GPU with ≥ 24 GB VRAM). The UI displays a **dismissible** warning banner when `false` — no functionality is blocked. Always `true` when `GAIA_SKIP_DEVICE_CHECK` is set or when `LEMONADE_BASE_URL` points to a non-localhost server (remote inference, no local hardware requirements).
  </Accordion>
