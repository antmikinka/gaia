...
|--------|------|---------|-------------|
| `--model` | string | "Qwen3.5-35B-A3B-GGUF" | Language model ID for AI processing (not a Blender 3D model) |
...
<Note>
**Positioning Note**: Relative positioning (e.g., "3 units above") may not always position objects as expected. For precise placement, use explicit 3D coordinates like `(0, 0, 3)` instead.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Blender MCP Server Issues">
  ...
  <Accordion title="Getting Help">
    ...
    2. Review the [Development Guide](/reference/dev#troubleshooting)
    3. Consult the [FAQ](/reference/faq) for common solutions
...
<Note>
**Output Directory Behavior**: The `--output-dir` creates the specified directory, but output files depend on the specific operations performed. Some queries may not generate output files even with this flag set.
</Note>
