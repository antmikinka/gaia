...
**Target Branch:** `main`
**Document Status:** Draft — active, updated 2026-04-11 with Session-3 resilience wiring and capability migration
...
| Resilience Patterns | `src/gaia/resilience/` | Provides `CircuitBreaker` (sub-10ms trip time), `Bulkhead` (concurrent request isolation), and `Retry` (configurable backoff strategies). Delivered in Phase 4 Week 2. **Session-3 (2026-04-11):** Resilience primitives wired into `RoutingEngine.route_defect_resilient()` with circuit breaker, bulkhead, and retry. |
...
#### Post-Merge Integration (P1)

3. **Security Boundary — resilience primitives NOW WIRED into pipeline engine call sites.** (Session-3, 2026-04-11) The resilience primitives (`CircuitBreaker`, `Bulkhead`, `Retry` in `src/gaia/resilience/`) are now wired into `RoutingEngine` (`src/gaia/pipeline/routing_engine.py`). The `route_defect_resilient()` method wraps routing with circuit breaker (failure_threshold=5, recovery_timeout=30s), bulkhead (max_concurrency=10), and retry (max_retries=3, exponential backoff). **Status: RESOLVED.** Resilient routing available via `engine.route_defect_resilient(defect)`; existing `route_defect()` unchanged for backward compatibility. Quality review: PASS (quality_review_session3.md).

4. **Capability vocabulary bifurcation RESOLVED.** (Session-3, 2026-04-11) Migration script `util/migrate-capabilities.py` created and applied. 3 capability strings updated across 2 YAML files: `security-auditor.yaml` (`compliance-check` → `compliance-audit`), `senior-developer.yaml` (`testing` → `test-automation`, `refactoring` → `code-refactoring`). The 5 Python pipeline stage agents already had aligned vocabulary via MD frontmatter. **Status: RESOLVED.** Unified vocabulary defined in `docs/spec/unified-capability-model.md`. Quality review: PASS.

**Session-3 B3-C Bug Fixes (2026-04-11):** SSE endpoint lock release logic simplified (removed `locks_released` tracking), JSON serialization error handling added to all streaming event paths. All 4 quality review bugs FIXED. Quality review: PASS.
...
#### Phase 6 Enhancements (P2, Post-PR #606)
...
#### Resolved in Session-2 (2026-04-10)

The following issues were identified and fixed in Session-2 (2026-04-10). All fixes are pending commit.

5. **Quality reviewer final coherence check — RESOLVED (Phase 6).** The design spec (`agent-ecosystem-design-spec.md`) Section 2.2 "What Is Missing" has been updated (commit `41ee396`) to reflect Phase 5 delivery: items 3, 4, and 5 now show "DELIVERED", "RESOLVED", and "PARTIALLY RESOLVED" respectively. The three Phase 5 spec documents (`phase5_multi_stage_pipeline.md`, `component-framework-design-spec.md`, `component-framework-implementation-plan.md`) all have YAML frontmatter and coherent content. **Status: RESOLVED.**

7. **YAML frontmatter missing from nine `docs/spec/*.md` files.** Nine specification files were committed to `docs/spec/` without YAML frontmatter. The Mintlify documentation framework requires YAML frontmatter (at minimum a `title` field) for `.md` files to render correctly in the documentation site. Without frontmatter, these files will either fail to render or be rendered with incorrect titles. **Status: CLOSED** (Phase 6 — 2026-04-08 — All 9 spec files received YAML frontmatter in commit 41ee396. Documentation build risk eliminated.)
...
**Additional Session-2 Fixes (not previously tracked as open items):**

- **BF-07 FIXED — `PipelineOrchestrator.execute_tool()` AttributeError:** Fixed `self.execute_tool(` → `self._execute_tool(` at `src/gaia/pipeline/orchestrator.py:491`. Commit `242e380`.
- **BF-08 FIXED — `tool_fn(self, **tool_args)` TypeError in five stage files:** Removed `self` from dispatch call in all five stage files. Commit `242e380`.
- **BF-12 FIXED — `gaia pipeline` CLI had no Lemonade readiness check:** Added `"pipeline": 32768` to `agent_context_sizes` and added `initialize_lemonade_for_agent("pipeline")` call. Pending commit.
- **BF-13 FIXED — PyPI exports for `PipelineOrchestrator` and `run_pipeline`:** Added to `src/gaia/pipeline/__init__.py` lazy loader and `__all__`. Pending commit.
- **B3-C RESOLVED — Agent UI pipeline integration complete:** Implemented `POST /api/v1/pipeline/run` SSE endpoint with full 5-stage progress streaming. Frontend `PipelinePanel` component, `pipelineStore.ts`, and API functions created. Users can now execute pipelines from Agent UI. See `B3-C-IMPLEMENTATION-BLUEPRINT.md`.
...
**Theme:** Resilience Wrapping describes the design intent for how `CircuitBreaker`, `Bulkhead`, and `Retry` primitives from `src/gaia/resilience/` should wrap external calls in the pipeline engine. This theme is documented here as a cross-cutting concern.
...
- `src/gaia/pipeline/routing_engine.py` — **Session-3 (2026-04-11):** `RoutingEngine.route_defect_resilient()` now wraps defect routing with circuit breaker (failure_threshold=5, recovery_timeout=30s), bulkhead (max_concurrency=10), and retry (max_retries=3, exponential backoff). Backward compatible: existing `route_defect()` unchanged.
...
**Current state:** The `connection_pool.py` integration is complete. The `routing_engine.py` integration is complete (Session-3). The `pipeline/engine.py` integration remains deferred pending the AgentOrchestrator (Open Item 1).

**Reviewer expectation:** The `RoutingEngine` now provides resilience-wrapped routing via `engine.route_defect_resilient(defect)`. Before merging the pipeline engine as a production feature, reviewers should confirm that agent call sites in `engine.py` and `loop_manager.py` are wrapped with the resilience primitives, or that the pipeline is documented as a development preview with known reliability limitations.
...
| `src/gaia/resilience/` (4 new files) | Resilience primitives: `CircuitBreaker`, `Bulkhead`, `Retry`. **Session-3 (2026-04-11):** Now wired into `RoutingEngine.route_defect_resilient()`. | Fully isolated new module. No existing GAIA code imports from `src/gaia/resilience/` on main. All 115 tests pass at 100%. | Session-3 resolved the wiring gap for routing. See Section 4.5 for details on `route_defect_resilient()` implementation. The module itself is low risk; partial wiring (routing only, not engine.py) is acceptable for merge. |
...
- **Session-3**: 2026-04-11 — Resilience wiring in `RoutingEngine` (+`route_defect_resilient()`), capability vocabulary migration (`util/migrate-capabilities.py`)
...
**Session-3 Completed (2026-04-11) — files modified in working tree:**

| Files Modified | Description | Category | Phase |
|---|---|---|---|
| `src/gaia/pipeline/routing_engine.py` | P1-1: Add `route_defect_resilient()` with circuit breaker, bulkhead, retry; imports from `gaia.resilience` | Pipeline Orchestration + Resilience | Session-3 |
