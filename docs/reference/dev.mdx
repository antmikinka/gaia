...
**Linux / macOS:**
```bash
./scripts/start-agent-ui.sh                # Start both backend + frontend
./scripts/start-agent-ui.sh --backend-only # Backend only (port 4200)
./scripts/start-agent-ui.sh --frontend-only # Frontend dev server only (port 5174)
```

**Windows PowerShell:**
```powershell
.\scripts\start-agent-ui.ps1               # Start both backend + frontend
.\scripts\start-agent-ui.ps1 -BackendOnly  # Backend only (port 4200)
.\scripts\start-agent-ui.ps1 -FrontendOnly # Frontend dev server only (port 5174)
```
