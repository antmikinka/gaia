...
Execute a natural-language task through the 5-stage autonomous agent pipeline.

<Note>
  **Auto-spawn requires Claude Code.** The `GapDetector` stage invokes `master-ecosystem-creator.md`, a Claude Code subagent, when gaps are detected. Running outside Claude Code will result in silent failures when gaps are found. Use `--no-spawn` to disable auto-spawn for standalone deployments.
</Note>

```bash
gaia pipeline "TASK DESCRIPTION" [OPTIONS]
```
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `--model` | string | Qwen3.5-35B-A3B-GGUF | Model to use for pipeline execution |
| `--no-spawn` | flag | false | Disable automatic agent spawning when gaps detected |
| `--debug` | flag | false | Enable debug output with stage-by-stage progress |

**Examples:**

<CodeGroup>
```bash Run Pipeline (Default)
gaia pipeline "Build a data analysis agent for CSV processing"
```

```bash Specify Custom Model
gaia pipeline "Create a code review agent" --model Qwen3.5-35B-A3B-GGUF
```

```bash Disable Auto-Spawn
gaia pipeline "Build testing agent" --no-spawn
```

```bash Show Pipeline Stage Diagram
gaia pipeline
```

```bash Debug Mode
gaia pipeline "Build multi-agent system" --debug
```
</CodeGroup>

**What It Does:**

1. **Initializes Lemonade Server** - Ensures LLM backend is ready via `initialize_lemonade_for_agent("pipeline")`
2. **Stage 1: Domain Analysis** - Analyzes task to identify knowledge domains
3. **Stage 2: Workflow Modeling** - Models workflow and recommends required agents
4. **Stage 3: Loom Building** - Builds execution topology
5. **Stage 4: Gap Detection** - Scans for missing agents, auto-generates if needed
6. **Stage 5: Pipeline Execution** - Executes complete agent pipeline

**Prerequisites:**

- Lemonade Server running on port 8000
- Clear Thought MCP server (optional, provides sequential thinking at each stage)
- Claude Code environment (required for auto-spawn via `master-ecosystem-creator.md`)

**Pipeline Stage Diagram:**

When run without a task argument, `gaia pipeline` displays the stage flow:
...
For advanced use cases, use the Python SDK directly:

```python
from gaia.pipeline.orchestrator import run_pipeline

result = run_pipeline(
    task_description="Build a data analysis agent",
    auto_spawn=True,
...
[Full Pipeline Guide](/guides/pipeline) | [Auto-Spawn Guide](/guides/auto-spawn-pipeline) | [SDK Reference](/sdk/infrastructure/pipeline)
