...
<Note>
  **Programmatic API only.** The `gaia pipeline` command is registered as a placeholder. The pipeline orchestration engine is accessed through the Python SDK — there is no interactive CLI workflow at this time.
</Note>
...
Running `gaia pipeline` prints SDK import examples and documentation links.
...
[Full Pipeline Guide](/guides/pipeline) | [SDK Reference](/sdk/infrastructure/pipeline)
...
<CardGroup cols={2}>
  ...
  <Card title="Pipeline Orchestration" icon="diagram-project" href="/guides/pipeline">
    Self-improving recursive AI workflows
  </Card>
</CardGroup>
