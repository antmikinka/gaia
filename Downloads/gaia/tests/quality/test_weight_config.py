...
class TestQualityWeightConfigManager:
    ...
    def test_create_custom_config_no_validation(self, manager: QualityWeightConfigManager):
        """Test creating custom config without validation."""
        config = manager.create_custom_config(
            name="unvalidated",
            weights={"code_quality": 0.90, "testing": 0.90},
            validate=False,
        )

        assert config.name == "unvalidated"
        # Config created without raising error
...
class TestYamlJsonLoading:
    """Tests for YAML/JSON loading functionality."""
    ...
    def test_load_from_yaml_file(self, manager: QualityWeightConfigManager):
...
"""
        with NamedTemporaryFile(mode='w', suffix='.yml', delete=False) as f:
            ...
            config = manager.load_from_yaml(f.name)

            assert config.name == "yaml_test"
            assert config.weights["code_quality"] == 0.30
            config.validate()

    def test_load_from_json_file(self, manager: QualityWeightConfigManager):
        ...
        with NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            config = manager.load_from_json(f.name)

            assert config.name == "json_test"
            assert config.weights["testing"] == 0.25
            config.validate()

    def test_load_from_yaml_file_not_found(self, manager: QualityWeightConfigManager):
        """Test loading from nonexistent file raises error."""
        with pytest.raises(FileNotFoundError):
            manager.load_from_yaml("/nonexistent/path/config.yml")

    def test_load_from_json_file_not_found(self, manager: QualityWeightConfigManager):
        """Test loading from nonexistent file raises error."""
        with pytest.raises(FileNotFoundError):
            manager.load_from_json("/nonexistent/path/config.json")

    def test_load_yaml_with_category_overrides(self, manager: QualityWeightConfigManager):
        """Test loading YAML with category overrides."""
        yaml_content = """
name: override_test
weights:
  code_quality: 0.25
...
"""
        with NamedTemporaryFile(mode='w', suffix='.yml', delete=False) as f:
            ...
            assert "code_quality" in config.category_overrides
            assert config.category_overrides["code_quality"]["CQ-01"] == 0.10

    def test_save_to_yaml(self, manager: QualityWeightConfigManager):
        ...
        with NamedTemporaryFile(mode='w', suffix='.yml', delete=False) as f:
            manager.save_to_yaml(config, f.name)
            f.flush()
            ...
            assert data["weights"]["code_quality"] == 0.40

    def test_save_to_json(self, manager: QualityWeightConfigManager):
        """Test saving configuration to JSON."""
        config = QualityWeightConfig(
            name="json_save_test",
            weights={"code_quality": 0.35, "testing": 0.65},
            description="JSON save test",
        )

        with NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            manager.save_to_json(config, f.name)
            f.flush()
            ...
            assert data["name"] == "json_save_test"
            assert data["weights"]["code_quality"] == 0.35
...
class TestQualityWeightConfigIntegration:
    ...
    def test_profile_roundtrip(self):
        ...
        with NamedTemporaryFile(mode='w', suffix='.yml', delete=False) as f:
            manager.save_to_yaml(original, f.name)
            f.flush()

            loaded = manager.load_from_yaml(f.name)
    ...
    def test_custom_config_with_overrides(self):
        ...
        config = manager.create_custom_config(
            ...
            category_overrides={
                "testing": {
                    "TS-01": 0.12,  # Higher weight for unit tests
                    "TS-02": 0.08,  # Integration tests
                },
                "best_practices": {
                    "BP-01": 0.10,  # Security practices weighted higher
                },
            },
        )
