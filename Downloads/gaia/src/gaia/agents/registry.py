...
class AgentRegistry:
    ...
    def __init__(
        ...
        self._get_agents_by_capability_cached = self._lru_cache_wrapper(self._get_agents_by_capability_impl)
        ...
        try:
            ...
            return AgentDefinition(
                ...
                triggers=AgentTriggers(
                    ...
                    complexity_range=(
                        tuple(triggers_data.get("complexity_range", {"min": 0, "max": 1}).values())
    ...
    def _build_indexes(self) -> None:
        ...
        # Invalidate LRU cache when indexes are rebuilt (QW-002)
            ...
            class AgentFileHandler(FileSystemEventHandler):
                ...
                def on_modified(self, event):
                    if event.src_path.endswith((".yaml", ".yml")):
                        asyncio.create_task(self.registry._reload_agent(Path(event.src_path)))
                ...
                def on_deleted(self, event):
                    if event.src_path.endswith((".yaml", ".yml")):
                        asyncio.create_task(self.registry._unload_agent(Path(event.src_path)))
    ...
    def _get_agents_by_capability_impl(self, capability: str) -> List[str]:
        """
        Internal method to get agent IDs by capability (for LRU caching).
    ...
    def invalidate_capability_cache(self) -> None:
        ...
        """
        if hasattr(self._get_agents_by_capability_cached, 'cache_clear'):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "categories": {cat: len(agents) for cat, agents in self._category_index.items()},
    ...
    def get_specialist_agent(
        ...
        - CODE_QUALITY -> quality-reviewer
        - REQUIREMENTS -> software-program-manager
        - ARCHITECTURE -> solutions-architect
        - ACCESSIBILITY -> accessibility-reviewer
        - COMPATIBILITY -> frontend-specialist
        - DATA_INTEGRITY -> backend-specialist, data-engineer
        ...
        # Last resort - return first available enabled agent
    ...
    def get_specialist_agents(
        ...
        Example:
            >>> agents = registry.get_specialist_agents(["SECURITY", "PERFORMANCE"])
            >>> print(agents)
            {'SECURITY': 'security-auditor', 'PERFORMANCE': 'performance-analyst'}
        """
