...
from gaia.metrics.benchmarks import BenchmarkResult, BenchmarkType, PipelineBenchmarker
...
class ScaleTestRunner:
    ...
    def __init__(self, output_dir: str = None):
        self._output_dir = (
            Path(output_dir)
            if output_dir
            else Path(__file__).parent.parent.parent.parent / "gaia-proposal"
        )
    ...
    async def run_scale_test(
        self, concurrency_level: int, iterations: int = 3
    ) -> ScaleTestResult:
        """
        ...
        """
        logger.info(
            f"Starting scale test at {concurrency_level} concurrent loops ({iterations} iterations)"
        )
        ...
        for iteration in range(iterations):
            ...
            for result in results:
                ...
                if isinstance(result, Exception):
                    iteration_errors += 1
                    iteration_latencies.append(
                        elapsed_ms / concurrency_level
                    )  # Estimate
                elif isinstance(result, dict):
                    iteration_latencies.append(
                        result.get("latency_ms", elapsed_ms / concurrency_level)
                    )
            ...
            if PSUTIL_AVAILABLE:
                ...
                peak_memory_mb = max(
                    peak_memory_mb, total_memory_mb - baseline_memory_mb
                )
            ...
            logger.debug(
                f"Iteration {iteration + 1}: {elapsed_ms:.2f}ms total, {len(iteration_latencies)} samples, {iteration_errors} errors"
            )
        ...
        total_duration_ms = (
            statistics.mean([r for r in all_latencies]) * concurrency_level / iterations
            if n > 0
            else 0
        )
        ...
        throughput = (
            concurrency_level / (total_duration_ms / 1000)
        ...
        logger.info(
            f"Scale test complete (level={concurrency_level}): throughput={throughput:.1f} loops/sec, p99={p99_latency:.2f}ms"
        )

        return result

    async def run_all_scale_tests(
        self, scale_levels: List[int] = None
    ...
    def identify_bottlenecks(self) -> List[BottleneckAnalysis]:
        ...
        if first_result.throughput_loops_per_sec > 0:
            scale_factor = (
                last_result.concurrency_level / first_result.concurrency_level
            )
            throughput_ratio = (
                first_result.throughput_loops_per_sec
                / last_result.throughput_loops_per_sec
                if last_result.throughput_loops_per_sec > 0
                else float("inf")
            )
        ...
        if len(self._results) >= 3:
            ...
            for result in self._results[1:]:
                expected_throughput = baseline_throughput * (
                    result.concurrency_level / baseline_concurrency
                )
    ...
    def generate_report(self) -> str:
        ...
        for result in self._results:
            bar_length = int(
                (result.throughput_loops_per_sec / max_throughput) * graph_width
            )
            bar = "#" * bar_length
            lines.append(
                f"  {result.concurrency_level:4d} loops: {bar} {result.throughput_loops_per_sec:.1f}/s"
            )
        ...
        p31_100_loop_result = next(
            (r for r in self._results if r.concurrency_level == 100), None
        )

        if p31_100_loop_result:
            throughput_change = (
                p31_100_loop_result.throughput_loops_per_sec - p31_baseline_throughput
            )
            throughput_change_pct = (throughput_change / p31_baseline_throughput) * 100
            status = (
                "IMPROVED"
                if throughput_change > 0
                else "DEGRADED" if throughput_change < -10 else "STABLE"
            )
            lines.append(
        ...
        max_stable_throughput = max(
            self._results, key=lambda r: r.throughput_loops_per_sec
        )
        ...
        if bottlenecks:
            ...
            for bn in bottlenecks[:3]:
                recommendations.append(
                    f"   - {bn.bottleneck_type}: {bn.recommendation}"
                )
        ...
        criteria = [
            (
                "Throughput > 100 loops/sec @ 100 concurrency",
                ">100",
                f"{next((r.throughput_loops_per_sec for r in self._results if r.concurrency_level == 100), 0):.1f}",
                (
                    "PASS"
                    if any(
                        r.concurrency_level == 100 and r.throughput_loops_per_sec > 100
            ...
            (
                "Error rate < 1% @ all levels",
                "<1%",
                f"{max(r.error_rate for r in self._results) * 100:.4f}%",
                "PASS" if all(r.error_rate < 0.01 for r in self._results) else "FAIL",
            ),
            (
                "Memory < 500MB @ max scale",
                "<500MB",
                f"{max(r.memory_peak_mb for r in self._results):.1f}MB",
                (
                    "PASS"
                    if all(r.memory_peak_mb < 500 for r in self._results)
                    else "FAIL"
                ),
            ),
            (
                ...
                "0 critical",
                f"{len([b for b in bottlenecks if b.severity == 'critical'])}",
                (
                    "PASS"
                    if not any(b.severity == "critical" for b in bottlenecks)
                    else "FAIL"
                ),
            ),
        ]
...
async def main():
    ...
    if bottlenecks:
        ...
        for bn in bottlenecks:
            print(
                f"    - [{bn.severity.upper()}] {bn.bottleneck_type} @ {bn.scale_level} loops"
            )
    else:
